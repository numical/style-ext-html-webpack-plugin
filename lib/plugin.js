...
const wirePluginEvent = (hook, fn) => {
  hook.tapAsync('StyleExtHtmlWebpackPlugin', (pluginArgs, callback) => {
    try {
      fn(pluginArgs);
...
class StyleExtHtmlWebpackPlugin {
  ...
  apply (compiler) {
    ...
    compiler.hooks.emit.tapAsync('StyleExtHtmlWebpackPlugin', (compilation, callback) => {
      if (cssFilename) {
