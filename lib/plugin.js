...
const onlyChunkFiles = (chunkNames, htmlWebpackPluginChunks, compilation) => {
  let matchingChunks = compilation.chunks.filter(chunk => chunkNames.includes(chunk.name));
  if (htmlWebpackPluginChunks) {
    matchingChunks = matchingChunks.filter(chunk => htmlWebpackPluginChunks.includes(chunk.name));
  }
  return matchingChunks.length > 0
    ? filename => matchingChunks.some(chunk => chunk.files.includes(filename))
    : NO_FILES;
};
