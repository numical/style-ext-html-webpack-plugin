    ...
    if (this.enabled) {
      ...
      compiler.plugin('compilation', (compilation) => {
        compilation.plugin(EVENT, (pluginArgs, callback) => {
          ...
          if(pluginArgs.head[0].tagName === 'link') {
            try {
              ...
              PROCESSED_FILES.push(cssFilename); // if we've replaced the link so far without failure, store a reference to the processed file
