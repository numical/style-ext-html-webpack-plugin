    ...
    compiler.plugin('compilation', (compilation) => {
      ...
      if (options.position === 'plugin') {
        wirePluginEvent(
          ...
          (pluginArgs) => {
            if (cssFilename) {
              replaceLinkTagWithStyleTag(cssFilename, pluginArgs, compilation, minifier, options.attrs);
            }
          }
        );
      }
      ...
      if (options.position !== 'plugin') {
        wirePluginEvent(
          ...
          (pluginArgs) => {
            if (cssFilename) {
              insertStyleTagInHtml(cssFilename, options.position, pluginArgs, compilation, minifier, options.attrs);
            }
          }
        );
      }
